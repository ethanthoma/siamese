...
class TrackerSiamFC(Tracker):
    ...
    def update(self, img):
        ...
        responses = np.stack(
            [
                cv2.resize(
                    u, (self.upscale_sz, self.upscale_sz), interpolation=cv2.INTER_CUBIC
                )
                for u in responses
            ]
        )
        ...
        disp_in_instance = (
            disp_in_response * self.cfg.total_stride / self.cfg.response_up
        )
        disp_in_image = (
            disp_in_instance
            * self.x_sz
            * self.scale_factors[scale_id]
            / self.cfg.instance_sz
        )
        self.center += disp_in_image

        # update target size
        scale = (1 - self.cfg.scale_lr) * 1.0 + self.cfg.scale_lr * self.scale_factors[
            scale_id
        ]
    ...
    def train_step(self, data, backward=True):
        ...
        assert not torch.isnan(batch[0]).any(), "NaNs in input z"
        assert not torch.isnan(batch[1]).any(), "NaNs in input x"
